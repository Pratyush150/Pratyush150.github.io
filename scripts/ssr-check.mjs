...
check(
  // The guarantee is "no clip is fetched until it is nearly on screen". That used
  // to be spelled preload="none", which also stopped the browser buffering once
  // the element *did* mount, so wells sat on their poster while the reader waited.
  // It is now spelled: the <video> only mounts behind `armed`, and warming runs
  // through the bounded queue in lib/prefetchQueue.ts. Assert that shape instead.
  'E24 · <video> is muted, playsInline, unlooped, and mounted only when armed',
  (() => {
    const mw = readFileSync(root('src/components/MediaWell.tsx'), 'utf8');
    return (
      /muted\s+playsInline/.test(mw) &&
      !/\bloop\b/.test(mw) &&
      /\{wanted && armed \?/.test(mw) &&
      /from '\.\.\/lib\/prefetchQueue'/.test(mw)
    );
  })(),
);
check(
  'E24 · the warm-up queue is bounded and idle-scheduled',
  (() => {
    const q = readFileSync(root('src/lib/prefetchQueue.ts'), 'utf8');
    return /MAX_IN_FLIGHT\s*=\s*[1-3]\b/.test(q) && /requestIdleCallback/.test(q);
  })(),
);
check(
