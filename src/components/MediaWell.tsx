...
import { markWarm, warm } from '../lib/prefetchQueue';
...
export default function MediaWell({
  ...
  useEffect(() => {
    ...
    // Queue the clip for a background warm-up so the bytes are usually already
    // cached by the time `armed` mounts the <video>. Bounded and idle-scheduled,
    // so warming the page does not starve the clip being watched.
    warm(assets?.webm ?? assets?.mp4, eager);
    return () => {
      near.disconnect();
      seen.disconnect();
    };
      ...
      <div ref={boxRef} className="well__box" style={{ aspectRatio: aspect }}>
        ...
        {wanted && armed ? (
          <video
            ...
            onLoadedData={() => markWarm(assets?.webm ?? assets?.mp4)}
          >
